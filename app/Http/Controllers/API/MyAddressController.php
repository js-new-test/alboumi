...
class MyAddressController extends Controller
{
    ...
    public function getCustomerAddressList(Request $request)
    {
        try {
            ...
            $customer_addresses = \App\Models\CustomerAddress::where('customer_id', $request->customerId)
    ...
    {
        //Localization
        $codes = ['ADDRESSIDREQ','ADDRESSNOTFOUND'];
        $addressLabels = getCodesMsg($request->language_id, $codes);
        ...
        $cust_address = \App\Models\CustomerAddress::where('id', $request->address_id)->where('is_deleted', 0)->first();
        if($cust_address)
        {
            $result['statusCode'] = '200';                                                              
            $result['addressList'] = $cust_address;
            return response()->json($result); 
        }
        else
        {
    ...
    public function deleteAddress(Request $request)
    {
        ...
        $customer_id = Auth::guard('api')->user()->token()->user_id;
        $cust_address = \App\Models\CustomerAddress::where('id', $request->address_id)
    ...
    {
        $country_list = \App\Models\Country::select('id', 'name', 'sortname', 'is_active', 'phonecode')
        ->where('id', 17)->where('is_active', 1)->whereNull('deleted_at')->get();
        $result['statusCode'] = '200';                                                              
