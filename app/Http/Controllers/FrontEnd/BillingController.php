...
class BillingController extends Controller
{
    public function showMyaddress()
    {
        ...
        'MYADDRESSSEODESC','MYADDRESSSEOKEYWORD','MYADDRESSES5','HOME','BILLINGADDRESS','addressType1','addressType2'
        ...
        $cust_addresses = \App\Models\BillingAddress::where('customer_id', $customer_id)->where('is_deleted', 0)->get();
        $cust_default_bill_address = \App\Models\BillingAddress::where('customer_id', $customer_id)->where('is_default', 1)->where('is_deleted', 0)->first();
        return view('frontend.billing-address', compact('baseUrl','countries','cust_addresses','pageName',
        'projectName','megamenuFileName','myAddressLabels','mobileMegamenuFileName','cust_default_bill_address'));
    }
    ...
    {           
        ...
        if($validator->fails()) {            
            return redirect('/customer/billing-address')
    ...
    public function changeDefaultAddress(Request $request)
    {
        $lang_id = Session::get('language_id');
        $customer_id = Session::get('customer_id');
        $customer_address = \App\Models\BillingAddress::where('id', $request->address_id)->first();
        if($customer_address)
        {
            if($request->is_default == 1)
            {
                $is_default = \App\Models\BillingAddress::where('id', $request->address_id)->where('is_default', 1)->first();            
                if(!$is_default)
                {
                    \App\Models\BillingAddress::where('id', '<>',$request->address_id)->where('customer_id', $customer_id)->update(['is_default' => 0]);            
