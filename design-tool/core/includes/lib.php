...
class lumise_lib{
	...
	public function addtocartMaster($customer_id){
		global $lumise;
		$db = $lumise->get_db();
		$db->prefix='';
		$date = @date ("Y-m-d H:i:s"/*, strtotime($date_old)*/);
		return $id = $db->insert('cart_master', array(
			'user_id' => $customer_id,
			'message' => $_SESSION['cartmaindata']['shoppingmsg'],
			'created_at' => $date,
		));

	    //$cart_get_query="select form cart where cart_master_id=""$cart_master_id and option_id=$_SESSION['cartmaindata']['option_id']";
	}
	...
	public function addtocart($data){
		...
		$db->prefix='';
		$date = @date ("Y-m-d H:i:s"/*, strtotime($date_old)*/);
		return $id = $db->insert('cart', $data);

	    //$cart_get_query="select form cart where cart_master_id=""$cart_master_id and option_id=$_SESSION['cartmaindata']['option_id']";
	}
	public function getecartdata($cart_master_id,$option_id){
		global $lumise;
		$this->main->db->prefix='';
		$query = sprintf(
			"SELECT * from cart where cart_master_id='".$cart_master_id."' and option_id='".$option_id."'"
				);

			$data = $this->main->db->rawQuery($query);
			return $data;
	    //$cart_get_query="select form cart where cart_master_id=""$cart_master_id and option_id=$_SESSION['cartmaindata']['option_id']";
	}
	public function get_productslug($id){
		global $lumise;
		$this->main->db->prefix='';
		$query = sprintf(
	...
	*/
	...
	public function remove_cart_item( $cart_id, $data = null ){
		...
		$this->main->connector->set_session('lumise_cart', array());
	}
