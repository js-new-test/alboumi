<?php
	...
	{
		$successUrl = ORDER_SUCCESS_URL . "?track_id=".$trackID."&payment_id=".$paymentID.$mobileVar;
	...
	else if ($myObj->getResult() == "NOT CAPTURED" || $myObj->getResult() == "CANCELED" || $myObj->getResult() == "DENIED BY RISK" || $myObj->getResult() == "HOST TIMEOUT")
	{
		if ($myObj->getResult() == "NOT CAPTURED")
		{
			...
			$cancelUrl = ORDER_CANCEL_URL . "?track_id=".$trackID."&error=".$response.$mobileVar;
	...
	else
	{
		...
		$cancelUrl = ORDER_CANCEL_URL . "?track_id=".$trackID."&error=".$errorText.$mobileVar;
