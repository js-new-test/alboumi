			...
			<div class="fb-right-side tab-content">
				...
				<div id="menu{{$key}}" class="tab-pane fade {{($key==0)?'in show active':''}}">
					<div class="mobile-upload-header">
						<h6>{{$row['album']}} (@if(!empty($row['photos'])){{count($row['photos'])}}@endif)</h6>
					...
					<div class="fb-imgs">
						<div class="row">
							@if(!empty($row['photos']))
							@foreach($row['photos'] as $key2=>$row2)
								<div class="col-6 col-sm-4 col-md-4 col-lg-3">
									<div class="fb-img-ck">
										<label class="ck">
										  <input type="checkbox" name="selectedImages" class="singleselect" value="{{$row2['source']}}">
										  <span class="checkmark"></span>
										</label>
										<img src="{{$row2['source']}}">
									</div>
