...
<section class="menu-section">
	<div class="container">
		<div class="row">
            @if(!empty($photographersData))
            @foreach($photographersData as $photo)
			<div class="col-6 col-sm-6 col-md-4 col-lg-3">
                <a href="{{ url('made-in-bahrain/'.$photo['id']) }}">
				    <img src="{{asset('public/assets/images/photographers/'.$photo['profile_pic'])}}">
                </a>
				<a href="{{ url('made-in-bahrain/'.$photo['id']) }}"><h6 class="text-center">{{ $photo['name']}}</h6></a>
            </div>
            @endforeach
            @endif
		</div>
