...
<title>Update Service | Alboumi</title>

@section('content')
<script type="text/javascript">
    var service_image = <?php echo json_encode($service['service_image']);?>;
            ...
            <div class="app-main__inner">
                ...
                <div class="main-card mb-3 card">
                    <div class="card-body">
                        ...
                        <form id="updateServiceForm" class="col-md-10 mx-auto" method="post"
                            action="{{ url('admin/services/updateService') }}" enctype="multipart/form-data">
                            @csrf
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div>
                                            <select
                                                ...
                                                <option value="{{ $language->id }}" {{ $service->language_id == $language->id ? 'selected' : '' }}>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <!-- <div>
                                            <input type="text" class="form-control" id="link" name="link"
                                                placeholder="Enter link" value="{{ $service->link }}" />
                                        </div> -->
                                        <select name="link" id="link" class="multiselect-dropdown form-control">
                                            @foreach($categories as $category)
                                                <option value="{{$category->parent_cat_id}}" {{($service->category_id == $category->parent_cat_id) ? 'selected' : ''}}>{{$category->title}}</option>                                            
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div>
                                            <textarea name="short_desc" id="short_desc" type="text" rows="4"
                                                class="form-control" placeholder="Write short description">{{ $service->short_desc }}</textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div>
                                            <input type="number" class="form-control" id="sort_order" name="sort_order"
                                                placeholder="Enter sort order" value="{{ $service->sort_order }}" />
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6">
